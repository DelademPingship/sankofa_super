...
- [ ] **Story:** As a user on the web experience, I want feature parity with mobile using the same backend APIs.
  ...
  - **Verification Steps:**
    - `cd sankofa_frontend && npm run build`
    - Launch `npm run dev` locally and walk through `/auth/register`, `/auth/login`, `/auth/kyc`, `/app/home` in Chrome + Safari.

- [x] **Story:** As a user on the web, I can register, log in, and verify my account using the same OTP flows as the Flutter app.
  - **Acceptance Criteria:**
    - `sankofa_frontend` exposes Ghana phone helpers shared by `authService`, login, and registration screens.
    - `/auth/register` mirrors the mobile flow (full-name capture, Ghana-formatted phone entry, optional email, OTP countdown/resend, and post-verification KYC branching).
    - `/auth/login` reuses the same OTP + countdown experience, including resend, invalid-number feedback, and a CTA that deep-links to sign-up when no account exists.
    - Successful OTP verification updates the shared `AuthContext` so downstream routes honor the authenticated session.
  - **Verification Steps:**
    - `cd sankofa_frontend && npm run build`
    - Start `npm run dev`, visit `/auth/register`, complete sign-up with a demo number, verify the OTP, and confirm you are routed to `/auth/kyc` when the user is pending KYC.
    - Sign out, visit `/auth/login`, request an OTP, verify it, and confirm you are routed to `/app/home` once the user is fully verified; use the resend action to ensure the countdown resets.
