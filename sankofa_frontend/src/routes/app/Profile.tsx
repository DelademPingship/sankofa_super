...
const Profile = () => {
  ...
  const fullName = user.fullName?.trim() || user.phoneNumber || 'Member';
  const avatarInitial = fullName.charAt(0)?.toUpperCase() || 'M';
  const walletBalance =
  ...
  const walletUpdatedAt = user.walletUpdatedAt ? new Date(user.walletUpdatedAt).toLocaleDateString() : '—';
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 text-sm text-slate-600 dark:text-slate-300 md:grid-cols-2">
            ...
            <div className="rounded-2xl bg-slate-50/80 p-4 dark:bg-slate-900/60">
              ...
              <p>GH?{walletBalance.toLocaleString()}</p>
            </div>
            <div className="rounded-2xl bg-slate-50/80 p-4 dark:bg-slate-900/60">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          {preferences.map((item) => (
            <div
              key={item.label}
              className="flex items-center justify-between rounded-2xl border border-slate-200/70 bg-white/80 px-4 py-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300"
            >
